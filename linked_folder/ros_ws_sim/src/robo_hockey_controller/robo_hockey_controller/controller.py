...
class Controller(PickupStateHandlers, HockeyStateHandlers, Node):
    ...
    def __init__(self, robot_id, role, sim=False):
        ...
        #reset the program and choose the first state to run
    ...
    def reset(self, _msg):
        ...
        self.arm_action_active = False
        self.gripper_action_active = False
        self.go_to_state('navigate_to_pickup_alignment')
    ...
    def wait_for_pose(self, name, pose, pose_time, initializing=False):
        ...
        if initializing: mocap_timeout = MOCAP_TIMEOUT_STARTUP
        else: mocap_timeout = MOCAP_TIMEOUT
