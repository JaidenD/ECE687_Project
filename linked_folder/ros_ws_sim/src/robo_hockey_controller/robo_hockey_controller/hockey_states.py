...
# from multi_robomaster_ros_sim.multi_robomaster_ros_sim.config import OTHER_ROBOT_SAFETY_DISTANCE
...
from robo_hockey_controller.helpers import (
    ...
    wrap,
    yaw_from_pose,
    check_segment_circle_intersection,
    is_point_in_rect
)
...
class HockeyStateHandlers:
    ...
    def hockey_state_handlers(self):
        ...
        return {
            ...
            'check_for_swing_completion': self.state_check_for_swing_completion,
        }
    ...
    def target_for_swing(self):
        ### step 1) find the target points
        if self.role == 'pass':
            if not self.wait_for_pose('scoring robot', self.other_robot_pose, self.other_robot_pose_time):
                return None
            target = np.array(self.receiving_point(self.other_robot_pose))
        else:
            if not self.wait_for_pose('goal', self.goal_pose, self.goal_pose_time):
                return None
            target = np.array([self.goal_pose.pose.position.x, self.goal_pose.pose.position.y]) + GOAL_TARGET_OFFSET_WORLD

        # time.sleep(2)  # small delay for debug
        ### step 2) check if the target intersects with any obstacles
        
        if self.role == "pass":
            p1 = np.array([self.robot_pose.pose.position.x, self.robot_pose.pose.position.y])
            stick_heading =  yaw_from_pose(self.robot_pose) + STICK_HEADING_OFFSET_FROM_ROBOT
            p1 = p1 + STICK_TIP_FROM_BASE * np.array([math.cos(stick_heading),math.sin(stick_heading)])
        else:
            p1=np.array([self.puck_pose.pose.position.x,self.puck_pose.pose.position.y ])
        # centers, safe_dists = self.navigation_obstacles(include_other_robot=(self.role != 'pass'), include_holder=True)
        centers, safe_dists = self.navigation_obstacles(include_other_robot=False, include_holder=True)
        # time.sleep(2)
        print(centers, safe_dists)
        #ps: do not include the other robot as obstacle for passing the puck
        # helper function: return True if path does not intersect any obstacles
        def is_path_clear(end_pt):
            return not any(
                check_segment_circle_intersection(p1, end_pt, c, r)
                 for c, r in zip(centers, safe_dists)
            )

        # if no intersect, return the target as it is
        # time.sleep(2)  # small delay for debug
        if is_path_clear(target):
            self.get_logger().info(f"Target {target} is clear of obstacles, proceeding.")
            return target
        
        # time.sleep(2)  # small delay for debug
        ### step 3) Rotate vector to avoid obstacles
        self.get_logger().info(f"Target {target} intersects with obstacles, searching for alternative path.")
        v = target - p1
        L = np.linalg.norm(v)
        dir_v = v / L
        target_distance = L / 2.0  

        #rotate by 5deg increments
        # time.sleep(2)  # small delay for debug
        angles_deg = [angle for i in range(5, 181, 5) for angle in (i, -i)]   
        for angle in angles_deg:
            self.get_logger().info(f"Checking path with rotation angle: {angle} degrees.")
            theta = np.radians(angle)
            c, s = np.cos(theta), np.sin(theta)
            v_rot = np.array([c * dir_v[0] - s * dir_v[1], s * dir_v[0] + c * dir_v[1]]) 
            
            full_dist = p1 + v_rot * 10 #arbitrarily far to ensure no obstacle intersect at full length
            new_target= p1 + v_rot * L/2 #place final target at halfway
            if is_path_clear(full_dist):
                return new_target
            
        # Fallback if no clear path found; return original target
        self.get_logger().warning("Warning: No clear path found to target without hitting obstacles, returning original target.")
        return target
    ...
    def state_navigate_to_swing(self, pose):
        ...
        puck_standoff_point= self.swing_plan['standoff_position']
        target_point = (
            self.swing_plan['standoff_position']
            + NAV_LOOKAHEAD_DIST * start_direction
        )

        self.get_logger().info(f'Puck Standoff Position: {puck_standoff_point}')
        self.get_logger().info(f'Navigate to Swing Target Point: {target_point}')
    ...
    def state_swing(self, pose):
        ...
        if progress >= total_angle:
            self.stop()
            self.get_logger().info(f'{self.role.upper()} Robot, Swing complete')
            self.go_to_state('check_for_swing_completion')
    ...
    def state_check_for_swing_completion(self, pose):
        '''check if puck position is in the goal area for scoring swing
                or if it's in near Robot 2 for passing swing'''
        x , y = np.array([self.puck_pose.pose.position.x, self.puck_pose.pose.position.y])
        xg, yg = np.array([self.goal_pose.pose.position.x, self.goal_pose.pose.position.y])
        if self.role == 'pass':
            if not self.wait_for_pose('other robot', self.other_robot_pose, self.other_robot_pose_time):
                return
            other_robot_position = np.array([self.other_robot_pose.pose.position.x, self.other_robot_pose.pose.position.y])
            distance_to_other_robot = np.linalg.norm(np.array([x, y]) - other_robot_position)
            if distance_to_other_robot < PUCK_RECEIVE_RADIUS:
                self.get_logger().info(f'Puck is near the receiving robot. Pass complete.')
                self.pass_complete = True
                self.go_to_state('pass_complete')
            else:
                self.get_logger().info(f'Puck is not near the receiving robot. Repeat Pass.')
                self.go_to_state('wait_for_both_pickups')
        else:
            if not self.wait_for_pose('goal', self.goal_pose, self.goal_pose_time):
                return
            if is_point_in_rect(np.array([x, y]), np.array([xg, yg]), yaw_from_pose(self.goal_pose), GOAL_WIDTH, GOAL_HEIGHT):
                self.get_logger().info(f'Puck is in the goal area. Score complete.')
                self.go_to_state('done')
            else:
                self.get_logger().info(f'Puck is not in the goal area. Repeat Scoring.')
